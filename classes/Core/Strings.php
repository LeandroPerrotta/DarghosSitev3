...
class Strings
{
	...
	{
		$query = \Core\Main::$DB->query("SELECT string FROM ".Tools::getSiteTable("blackliststrings")."");
	...
	{
		switch(Configs::Get(Configs::eConf()->USE_ENCRYPT))
		{
			case Consts::ENCRYPT_TYPE_MD5;
				$enc = md5($string);
			break;	
		}
	...
	static function canUseSecretKey($key)
	{
		...
		$temp = preg_match_all("([a-zA-Z])", $key, $matches);		
		if($temp != strlen($key))
			return false;	
