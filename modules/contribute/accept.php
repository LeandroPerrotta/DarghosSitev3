...
if($strings->SQLInjection($_GET['id']) and $contribute->load($_GET['id'], "id, target, period, target_account, status") and $contribute->get("target_account") == $_SESSION['login'][0] and $contribute->get("status") == 1)
{
	if($post)
	{
		...
		else
		{
			...
			<p>Agora sua conta já possui status de Contra Premium, o que lhe permitirá muitas novas possibilidades dentro do Darghos!</p>
...
<form action="" method="post">
	<fieldset>
		...
		<p>
			<input name="accept_terms" type="checkbox" value="1" /> Eu aceito com as clausulas e termos de contrato de contribuições do Darghos.
		</p>			
