 ...
 * RetornoPagSeguro
 *
 * Classe de manipula��o para o retorno do post do pagseguro
 *
 * @package PagSeguro
 */
class RetornoPagSeguro {
   ...
   *
   * @internal � usado pela {@see RetornoPAgSeguro::verifica} para gerar os,
   * dados que ser�o enviados pelo PagSeguro
   *
   * @access private
   *
   ...
   * @return array                        Array com as configura��es
   *
   */
  function _tipoEnvio() {
  ...
  function verifica($post, $tipoEnvio=false) {
    ...
    if (!in_array($tipoEnvio[0], array('curl', 'fsocket'))){
    	...
    	trigger_error("Tipo de envio desconhecido... Processo abortado.", E_WARNING);
    	return false;
    }
    $confirma = false;
    if ($tipoEnvio[0] === 'curl') {
    ...
    if ($confirma && method_exists("Controllers\Balance", "retorno_automatico")) {
      ...
      Controllers\Balance::retorno_automatico (
        ...
        $post['CliEstado'], $post['CliCEP'], $post['CliTelefone'], $produtos, $post['NumItens'], $key
      );
    }
    else
    	trigger_error("Fun��o retorno_automatico n�o encontrada ou dialogo nao autorizado...", E_WARNING);
...
if ($_POST) {
  trigger_error("Processando callback pagseguro...", E_NOTICE);
